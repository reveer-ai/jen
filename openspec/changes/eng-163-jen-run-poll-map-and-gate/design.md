...
**Every bound reports its own truncation, and that is a requirement of the report rather than a nicety.** A bounded page with no `hasNextPage` beside it cannot tell a short answer from a cut-off one, so a project holding more issues in stage statuses than the page reads would lose the overflow entirely — not dispatched, not declined, not named, nothing raised, exit 0. That is indistinguishable from a healthy quiet pipeline, which is the one failure shape this whole path is built to refuse, and it is *less* visible than the epic case the label gate was made a gate for: the epic at least gets a line. The tick does not page these connections and does not need to; a `note` line naming the bound and the flag discharges what the report owes, where paging would make the tick's cost scale with a project's backlog. The same holds one query up, where a truncated status page would otherwise produce a refusal — "the team carries no `Pending`" — that is simply untrue.
...
So the page is made to carry its own evidence: the client compares the first and last `createdAt` in what came back. Descending, and a full page is the newest — done, at no cost. Ascending, and a full page is the *oldest*, so the tick pages that issue's comments through to the newest before testing it. The same fallback covers the case it was originally written for, a page of markerless comments. This is a comparison, not a judgment, so it costs the dispatch path none of its predictability.

**The fallback carries a ceiling, because neither of its two walks ends on its own in the case that matters.** Walking backward, the walk stops at the first marker — cheap for a task that has been through a session, unbounded for one nothing has ever announced against, which has no marker anywhere to stop at. Walking forward it cannot stop early at all, since stopping early is the stale-marker bug the second walk exists to avoid; so if the connection really is ascending, every issue past one page re-reads its whole history on every tick, growing with the discussion rather than settling. Five further pages per issue per tick is the bound, and running out of it **declines the candidate as unproven rather than dispatching it**: "nothing is working this" is what starts a session, so an unfinished record must never read as that. The report names the issue and the flag, every tick, which is a visible stall rather than a silent duplicate session.
...
**A task with a long discussion since its last session costs up to five extra requests, and past that is not dispatched at all** → The nested comment page is bounded, so a task carrying more recent comments than the page holds needs follow-up reads to find its marker. Bounded to that one issue and to five pages of it, and a task whose record runs past that is declined as unproven rather than dispatched on an unread record. The visible cost is a task that stalls in the report until an operator raises `--comment-page`; the alternative costs are worse in both directions — a larger nested page pays complexity on every tick against every issue to serve the rare one, and an uncapped walk pays the rare one's whole history on every tick forever.
