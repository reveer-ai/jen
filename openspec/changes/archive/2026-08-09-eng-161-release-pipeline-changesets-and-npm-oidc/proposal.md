...
- **A GitHub App carries the release identity, and `main` gains a required status check.** These are one decision, not two. `GITHUB_TOKEN` cannot trigger workflows by design, so a Version PR it opened would never produce a check run — and a required check that never reports makes the PR **permanently unmergeable**. An App installation token does trigger workflows. Requiring checks on `main` is therefore only possible with the App, and requiring them closes the larger gap that nothing gates `main` at all today.

  The alternative — enabling *"Allow GitHub Actions to create and approve pull requests"* and using `GITHUB_TOKEN` — is rejected on privilege as well as capability. That toggle is a blanket grant to every workflow in scope, and the dangerous half is `approve`: it is harmless here only because the ruleset currently requires zero approving reviews, which is an argument from a present weakness rather than a property worth relying on. The App is a scoped identity holding exactly `contents: write` and `pull-requests: write`, installed on one repository. The toggle stays **off**.

- **The publish job runs in a restricted GitHub environment, and npm is told to require it.** A `release` environment, its deployment branches restricted to `main`, named both on the job and in the trusted-publisher entry. GitHub puts the environment in the OIDC token's claims and npm matches it, so the workflow filename stops being the only thing standing between a branch and the registry. Defense in depth against the case the filename check alone cannot catch.
...
- **Blocked on account-level setup, and not by oversight.** The pipeline needs configuration no workflow can create for itself: a GitHub App with two permissions and its credentials stored, a required status check on `main`, a `release` environment restricted to `main`, an npm organization owning the package, the `0.0.0` bootstrap publish, and a trusted-publisher entry. The workflow can be written, reviewed, and merged without any of it, but neither half of the flow — the Version PR opening, or merging it publishing — can be observed until it exists. This goes on the issue as a handoff rather than being quietly counted as passing.
