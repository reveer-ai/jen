...
Undetermined is the report where nothing has established the setting's reach. It is *not* the report where something has: this change buys the answer for the one setting that is on by default everywhere, and `setup-jen` ships that answer as a cited observation — host, date, vehicle, and the state it was made against — so a run meeting the setting reports it settled and names its provenance. The two are different reports because they rest on different things, and collapsing them either way is a defect: reporting undetermined where evidence exists blocks every adopter's first binding, and reporting settled where none does is the assumption this section refuses. See *Risks* below.
...
**Every first binding meets the unattributed setting, and on the rule as first written can never clear it.** → The chain is entirely inside this change. The setting is on by default on every ruleset; its reach cannot be read off the API; what the section says settles it is a pull request the pipeline itself opened — which a project *being bound* has never had, binding being the prerequisite for having one; and an undetermined setting leaves the gate unsatisfied. So every adopter's first run reports the gate unsatisfied and the project not ready, with no move available that changes it. Unlike the release-approval consequence above, this one cannot be accepted as written: it does not make the check stricter, it makes the check's report useless.

Resolved by shipping the observation, not by relaxing the rule. This change buys the answer once — one approval, not two, on an application-authored pull request — and `setup-jen` now carries it as a **cited observation**: host, date, vehicle, and the repository state it was made against. A run meeting that setting reports it settled by that observation and quotes the date; a setting with no observation behind it stays undetermined and still holds the gate unsatisfied, which is the case the rule was written for. The adopter's own first pipeline pull request re-settles it against their repository if they want it established there rather than borrowed.

The distinction between citing an observation and restating it is what keeps this from being a relaxation, and it is worth stating plainly because the whole task turns on it. Documentation is evidence about a setting; an observation is evidence about an implementation at a moment. Either one promoted into a standing assumption is the defect this task's own history is made of — a check on an input to a capability rather than on the capability. So what ships is the observation with its provenance attached and the instruction to cite rather than conclude, and a reader can weigh whether it still holds. *Residual risk:* GitHub changes the scoping — it calls the feature preview and subject to change — and an adopter reads a stale citation. Bounded by the date being on the face of it, by the setting's known direction of failure being visible (delivery parks rather than merges unreviewed), and by the adopter's own pull request re-settling it. Smaller than a gate check that reports every project not ready.
