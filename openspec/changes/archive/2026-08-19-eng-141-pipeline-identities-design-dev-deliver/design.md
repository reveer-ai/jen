...
| `deliver` | `contents:write`, `pull_requests:write`, `checks:read`, `statuses:read`, `workflows:write` | reviews and merges, reads whether checks passed, pushes the archive and spec sync |
...
**"Reads whether checks passed" is two permissions, not one.** The host reports a pull request's status through two independent mechanisms — check runs, under the `checks` permission, and commit statuses, under the separate `statuses` permission — and a ruleset's `required_status_checks` accepts contexts from either. `deliver` therefore holds both. Holding only `checks:read` reads clean on jen itself, where `check` is an Actions workflow producing a check run, and fails silently on an adopter whose CI posts through the statuses API: `deliver` sees nothing failing while something is, attempts the merge, and the host refuses it. That is a task stuck in delivery with delivery believing the merge should have worked — the expensive failure shape, because nothing in the output names the cause. This is the same two-mechanism trap as rulesets versus classic branch protection, one section below, and it is worth stating twice because the first read succeeds in both cases and only the answer is wrong.

jen's own three installations do not carry `statuses:read` today — they were registered against the earlier table, the same way they were a `workflows:write` short of it before task 1.4. It is invisible here, because jen's `check` is an Actions workflow and produces a check run, so nothing on this repository reads wrong for want of it. **ENG-173 carries the grant**, alongside raising the ruleset: that is the task where jen's own gate becomes load-bearing, and a permission for reading whether checks passed matters exactly when something gates on the answer. Recording it against a task rather than as an unchecked box here is deliberate — an unchecked box stops delivery rather than deferring anything.
...
**Steps 3 and 4 are ENG-173, not this change.** They were originally this change's task group 6, deferred but still written here as unchecked boxes — which would have stopped delivery rather than merely recording a deferral, because archiving counts unchecked tasks and the delivering stage is instructed to stop rather than confirm through. A migration plan is a record of intent and is archived with the change; a task is held by the board. So the ordering below is stated here as the reasoning, and ENG-173 carries the work, blocked by ENG-163 and ENG-164.
