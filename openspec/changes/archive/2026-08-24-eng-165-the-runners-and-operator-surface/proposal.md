...
- The **kill switch is the tracker's own project status.** A project carrying a status named `On Pause` halts dispatch: the tick refuses before it polls, reports why, and launches nothing. Both runners read it from the check they already make before polling, it needs no new concept, and toggling it is two clicks where the operator already is. A project the tracker considers `completed` or `canceled` halts for the same reason, matched on the type rather than on any name. (Designed as three halting types; `paused` turned out not to be one, and `design.md` records what replaced it and why.)
