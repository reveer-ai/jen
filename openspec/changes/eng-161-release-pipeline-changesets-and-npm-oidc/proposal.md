...
- **`setup-node` is used without `registry-url`.** Setting it writes an `.npmrc` containing `_authToken=${NODE_AUTH_TOKEN}`, and npm prefers that placeholder token over an OIDC exchange — surfacing as a 404 that reads like the package does not exist ([npm/cli#8730](https://github.com/npm/cli/issues/8730)). There is no token to place, so the option earns nothing and can only mislead. This is a deliberate departure from the reference workflow, which sets it.
...
- **A GitHub App is not stood up; the repository's Actions policy is opened instead.** The reference workflow uses an App so the Version PR triggers CI, which `GITHUB_TOKEN` cannot do by design. That is not the only thing an App buys, and the other thing is a hard blocker: **"Allow GitHub Actions to create and approve pull requests" is currently off**, and despite its name it governs *creating* a PR, not only approving one. `pull-requests: write` does not substitute — the setting and the permission are both required — so `GITHUB_TOKEN` cannot open the Version PR until the toggle is enabled at the organization and then at the repository. App installation tokens are not `GITHUB_TOKEN` and are unaffected by it, which is why the reference sidesteps this without remarking on it.
...
- **Nothing publishes that has not passed — and this is the release job's own doing.** Build, typecheck, and tests run as steps before the changesets action, so a broken `main` fails the release job rather than shipping. `ci.yml` is *not* the gate and cannot be: it fires on the same push and runs concurrently with `release.yml`, so it can still be red while a publish succeeds. The in-job checks are what make the missing CI on the Version PR affordable.
...
- **Modified**: `package.json` gains `repository` and the release scripts. `ci.yml` is untouched — the Node floor it pins is deliberately not the version the release job runs.
...
- **Noted, not fixed here**: `changeset publish` reaching npm through a script wrapper has broken OIDC env-var propagation for at least one toolchain ([npm/cli#8976](https://github.com/npm/cli/issues/8976)). The reference workflow publishes a scoped public package this exact way on npm and pnpm and works, so this follows it — but it is the first thing to suspect if the publish step authenticates as nobody, and the fallback (let the action open the PR only, publish in a top-level step) is recorded in design.
