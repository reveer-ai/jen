...
A project the tracker considers over — status type `completed` or `canceled` — halts dispatch, read by `type` so that a workspace which renamed those statuses still halts. A project the operator has *paused* halts too, and that one is a status named `On Pause` that jen prescribes and the operator creates under the `In Progress` category, matched by name.

*Revised during implementation, and the original is worth keeping visible because the revision is not a preference.* This was designed as three halting **types** — `paused`, `completed`, `canceled` — read by type rather than by name so a renamed status still halts. Testing established that `paused` is not a type: the tracker has five categories, `paused` is not among them, and the pause its older `state` field carried became a status *named* `Paused` filed under `planned` — the one category that must never halt, since ordinary planning projects share it. There was no name to add to the list. Filing the pause under `Completed` or `Canceled` would have preserved type-matching intact, and was rejected: it buys a working match by making the tracker assert on every surface that a live project is finished or abandoned.

So the pause is filed where it reads truthfully, under `In Progress`, and that category is shared by every working project and can carry no signal. The name is what is left. This is a narrower concession than it looks — jen already matches `In Design`, `In Progress`, and `Pending` by prescribed name, verified at bind time. The rule that survives is *what jen prescribes may be matched by name; what the workspace chose is matched by type*. What it costs is that renaming this one status disables the halt silently, which is why the docs and `setup-jen` both state it.

This needs no new concept, both runners read it in the pre-poll check they already make, and it is two clicks where the operator already is — after the one-time cost of creating the status, which no runner and no API call can do for them. It is a deny list over named types rather than an allow list over `started`, deliberately: jen's own project sits in `Backlog` while its pipeline runs, and an allow list would have halted it silently.
