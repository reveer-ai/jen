...
- [ ] 4.2b State that a managed installation's policy can refuse to mint one at all, so an adopter meeting that refusal reads it as a limit on their installation rather than as a malfunction, and knows the API key is still open to them.
