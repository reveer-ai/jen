...
**The delete is not reachable today, and is still right.** `credentialsFor` and `childEnvironment` read the same environment, and the former refuses when both names are set, so the latter cannot in practice meet a base holding the unheld name. Keeping the delete makes the unit's contract true on its own terms — *a session holds one model credential, the one the run holds* — rather than true only because a different function checked first. `childEnvironment` is unit-tested against constructed environments, so the property is directly assertable there, and the assertion is worth more than the line costs.
