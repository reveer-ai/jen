...
- [ ] 3.4 Add merge gate handling: verify the default branch, present the exact change when it is absent or insufficient, apply it only on explicit confirmation, and report it outstanding when declined. **Reopened — the definition of "sufficient" changed under it.** The first implementation encoded a two-condition gate (a count *and* approval postdating the last push); the gate is now the count alone, because the second condition is unsatisfiable once delivery pushes before merging. Restate the section around one condition, and add the two settings that must stay off — last-push approval and dismissal of stale reviews on push — with what each does to delivery, so a future run reads them as hazards rather than as the obvious next tightening
...
- [ ] 3.7 Update the skill's permission table with `Workflows: write` on `dev` and `deliver`, and say why it is not excessive: the refusal names the permission rather than the cause, and it must be granted even to a project with no `.github/workflows/` today, or the first task that adds one fails at push having done everything else right
- [ ] 3.8 State in the skill, where the gate is reported, that one approval from anyone with write access is all the host can be asked for — and that which role approves is workflow convention, not configuration. A user who believes the branch enforces which identity approves will never look at the timeline, and looking is the only detection there is
...
- [ ] 5.1 State in the root `AGENTS.md` conventions that only `review-task` approves and only `deliver-task` merges. Say why `implement-task` specifically is the one being restrained — `design-task` cannot approve in any case, since the host refuses a review from its own pull request's author, while `dev` holds `pull_requests:write` and is excluded by nothing on the branch — and say that this is the one workflow rule the host does not enforce for us
- [ ] 5.2 Confirm the prohibition is stated in exactly one place. Two copies drift, and the copy a stage happens to read is then a coin toss: `grep -rn 'only review-task' .claude/skills/ AGENTS.md` should find the conventions entry and nothing under `.claude/skills/`
