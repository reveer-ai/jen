...
**The pipeline's review path depends on the stages reaching the git host directly** → Resolved from a risk into a known dependency. The identities this change registers cannot produce a working verdict until the stage skills move their pull-request calls to `gh`, because the diff tools are invisible to the agent identity. Registering the identities is still independently useful and independently correct, but the epic needs both, and ENG-141 delivering alone leaves the gate satisfiable only by a human. Ordering: the merge gate should not be tightened on any repository until the stages there issue those calls through `gh`, or delivery blocks on an approval no stage can give.

**Key that ordering on the capability, not on ENG-166's status.** ENG-166 nominally owns the move and has already merged, as `1280f80`, *without* making it — the calls are still in `review-task`, `test-task`, and the root workflow document. A check written as "has ENG-166 landed" therefore passes today while the capability it stands for does not exist, and a run trusting it would tighten the ruleset onto a pipeline that cannot submit a verdict at all. An issue's status is a claim about intent; the skills' text is the evidence.
...
3. **Wait until the stage skills issue their pull-request calls through `gh`** — verified by reading the skills, not by reading ENG-166's status. Until then no stage can submit a verdict at all.
