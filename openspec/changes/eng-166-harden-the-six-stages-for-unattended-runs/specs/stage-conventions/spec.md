...
A git host MAY refuse a verdict from the pull request's own author. Before choosing the event, a stage SHALL compare the pull request's author to the identity it is authenticated as. Where they are the same, the stage SHALL record the verdict as a plain review comment stating the verdict in its body. Where they differ, it SHALL record the real event.

The determination SHALL be made from the author and the authenticated identity, both of which the host reports, and SHALL NOT be made by interpreting the text of a refusal: a refused self-review and a transport failure are indistinguishable from the message alone.
...
- **WHEN** a stage submits a verdict on a pull request it is itself the author of
