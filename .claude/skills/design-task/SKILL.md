...
**Done when** every artifact exists, `openspec validate <name> --strict` passes, and no unresolved threads remain. Leave the task at `In Design` — moving it to `In Progress` starts implementation, and that call is the user's. Leave the PR a draft too; marking it ready is implement-task's, once there's an implementation to review. Your end-of-session comment is the only thing distinguishing a finished design from an interrupted one, since both leave the task at the same status.
