...
- **It resolves to nothing.** Report the actor as **unattributed**, name the identifier you could not resolve, and do not report the gate as satisfied while one stands. This is the same third answer as an undetermined setting above and it behaves the same way: not a soft yes, and never grounds for calling the gate satisfied on the strength of the rest of the read. Reporting an unattributed actor as not-a-role is the unsafe default, and it is precisely the reading that produces green checks over an inert requirement.
